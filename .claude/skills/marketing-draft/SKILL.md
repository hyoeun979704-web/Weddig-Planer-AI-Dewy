...
| `naver_post` | **me**(1인칭 경험 / 네이버는 **해요체**) | 네이버블로그 | 상세 큐레이션·체크리스트 (네이버 SEO) |
...
- **어투(register)는 채널이 결정**: **me 반말은 쓰레드 한정** — 네이버 블로그는 같은 me라도 **친근 존댓말(해요체, ~했어요/~예요)**. 1인칭·경험담은 유지하되 반말 금지(§0 register_rule). 대표는 전 채널 존댓말.
