...
const APP_URL = process.env.DEWY_APP_URL ?? "https://dewy.kr";
const APP_LOGO_URL = process.env.DEWY_LOGO_URL ?? `${APP_URL}/dewy-logo.png`;
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === "GET") {
    return new Response(
      JSON.stringify({ ok: true, name: "kakao-chatbot" }),
      { headers: jsonHeaders },
    );
  }
