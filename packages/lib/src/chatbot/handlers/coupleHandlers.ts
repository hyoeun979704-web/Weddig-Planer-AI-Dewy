...
export const handleVotes = async (userId: string): Promise<string> => {
  ...
  if (total >= 5 && decidedPct >= 80) {
    ...
    insights.push(` **결정률 ${decidedPct}%** — 미결정이 많이 쌓였어요. 가장 임박한 항목부터 둘이서 한 번에 정리해보시는 게 좋아요.`);
  }
  ...
  let result = `진행 중인 투표 ${pending.length}건 · 결정된 투표 ${decided.length}건  (결정률 ${decidedPct}%)`;
