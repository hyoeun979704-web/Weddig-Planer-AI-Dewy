...
### 설치형 게임 (네이티브 창)
브라우저가 아니라 **데스크톱 창**으로 뜨는 tkinter 게임. tkinter 는 파이썬에 기본 포함
(Windows python.org 설치본 포함)이라 추가 설치가 없다. 작업 토큰(📋)이 책상 사이를 이동.
```
python office_game.py            # 게임 창 실행
python office_game.py --selftest # 잠깐 띄웠다 종료(점검)
```
...
### 승인 큐 · 섀도 모드 · 자동 검수 (P1 — 청사진 로드맵)
개입을 "큐 1클릭 승인"으로 축소(`docs/260613_agent_org_blueprint.md` P1).
- **섀도 모드**: 모든 산출물 기본 상태 `pending` — 승인 전엔 미발행. 대시보드 산출물 패널에서
  **✅ 승인 / ✖ 반려** 1클릭. 상태는 `queue.json`(런타임).
- **자동 검수(deslop)**: 마케팅 초안 저장 시 `deslop.py` 가 AI 티(em-dash·과장어휘·군더더기 도입부)
...
#### 포트레이트 조명 프리셋 (인물/커플 사진)
고전 인물 조명 10종을 프롬프트에 주입(`lighting.py`):
```
python visual_pipeline.py --list-lighting                       # 목록
python visual_pipeline.py "성수동 야외 커플 화보" --lighting rembrandt
```
rembrandt(램브란트)·loop(루프)·split(스플릿)·butterfly(버터플라이)·monster(몬스터)·
godfather(갓파더)·clamshell(클램셸)·rim(림)·broad(브로드)·short(숏).
