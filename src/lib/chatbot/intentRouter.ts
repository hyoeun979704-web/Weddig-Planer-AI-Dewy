...
const PATTERNS: IntentPattern[] = [
  ...
  // 통계·검색 (자유 텍스트)
  {
    ...
    patterns: [
      /(강남|강북|서초|마포|용산|종로|부산|대구|인천|성남|수원|천안|청주|경기|제주).*(식장|웨딩홀|스튜디오|드레스|메이크업|베뉴|로케이션|스냅|네일)/,
      // 기타 카테고리(스냅·DVD·네일·축가·부케 등)도 places 즉답으로 — LLM 환각 차단.
      /(식장|웨딩홀|스튜디오|드레스샵|메이크업|베뉴|로케이션|촬영지|촬영\s*장소|스냅|DVD|dvd|네일|축가|부케|브라이덜샤워).*추천/,
      /(찾|검색).*(웨딩홀|식장|스튜디오|베뉴|로케이션|스냅|네일|축가|부케)/,
      /(스냅|네일|축가|부케|DVD|dvd|브라이덜샤워).*(찾|검색|섭외)/,
