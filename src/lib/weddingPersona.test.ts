...
describe("derivePersonaMode — 도달성(모든 자동분류 모드가 어떤 입력으로든 나온다)", () => {
  const reaching: Record<WeddingPersonaMode, PersonaInputs> = {
    ...
    remote_overseas: base({ country: "US", wedding_country: "KR" }),
  ...
  it.each(ALL_MODES)("%s 모드에 도달 가능", (mode) => {
    expect(derivePersonaMode(reaching[mode])).toBe(mode);
  });

  // international(예식 자체가 해외/이중식) vs remote_overseas(해외 거주이나 예식은 한국).
  it("international(예식 해외) vs remote_overseas(해외거주·한국식) 구분", () => {
    expect(derivePersonaMode(base({ wedding_country: "US" }))).toBe("international");
    expect(derivePersonaMode(base({ ceremony_type: "dual_ceremony" }))).toBe("international");
    // 해외 거주(country≠KR) + 예식은 한국(wedding_country=KR) → remote_overseas
    expect(derivePersonaMode(base({ country: "US", wedding_country: "KR" }))).toBe("remote_overseas");
    // 해외 거주 + 예식도 해외 → 예식이 해외이므로 international 우선
    expect(derivePersonaMode(base({ country: "US", wedding_country: "JP" }))).toBe("international");
  });
