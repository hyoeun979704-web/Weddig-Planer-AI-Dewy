...
// ── Google Play 인앱결제(IAP) 엔진 — CdvPurchase(cordova-plugin-purchase v13) 래퍼 ──
// 웹 번들을 깨지 않으려고 정적 import 대신 전역(window.CdvPurchase)으로 접근한다(네이티브에서만 존재).
// 서버가 단일 진실원천: 구매 승인(approved) → 서버 영수증검증(iap-verify-google)에서만 하트/구독을
// 지급하고, 성공 후에만 transaction.finish()(소비성 consume/구독 acknowledge). 클라 결과는 신뢰하지 않음.
// ⚠️ 실제 동작은 실기기 + Play 라이선스 테스터 + 내부테스트 트랙에서만 검증 가능(컨테이너 불가).
// 설계: docs/260620_payment_compliance_plan.md, 등록: docs/260620_google_iap_setup.md.
...
/** 안드로이드 네이티브 + 플러그인 존재 시에만 IAP 사용 가능. */
export const isIapAvailable = (): boolean => getPlatform() === "android" && !!getCdv();
...
async function verifyOnServer(t: CdvTransaction): Promise<PurchaseResult> {
  ...
  if (!productId || !purchaseToken) return { ok: false, message: "구매 정보를 읽지 못했어요." };
  const kind = productId === SUBSCRIPTION_PRODUCT_ID ? "subscription" : "hearts";
...
export async function initIapStore(userId: string): Promise<void> {
  ...
  store.applicationUsername = userId; // Google obfuscatedAccountId 로 전달 → 서버 검증 시 대조.

  store.register(
    allProductIds().map((id) => ({
      id,
      type: id === SUBSCRIPTION_PRODUCT_ID ? ProductType.PAID_SUBSCRIPTION : ProductType.CONSUMABLE,
...
/** 구독 IAP 구매(자동갱신). type→base plan offer 선택. trial 은 월간 base plan(무료체험 offer). */
export async function purchaseSubscriptionIap(userId: string, type: SubscriptionType): Promise<PurchaseResult> {
  const cdv = getCdv();
  if (!cdv) throw new Error("결제 모듈을 사용할 수 없어요.");
  await initIapStore(userId);
  const product = cdv.store.get(SUBSCRIPTION_PRODUCT_ID, cdv.Platform.GOOGLE_PLAY);
