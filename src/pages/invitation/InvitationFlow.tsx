...
const InvitationFlow = () => {
  ...
  // ─────────────────────────────────────────────
  // 일러스트 변환 — auto_illustration 슬롯의 사진을 gpt-image-2 로
  //   흑백 라인 일러스트로 변환. applyCutoutToSlots 와 동일 구조.
  //   같은 source_path 는 Edge function 측에서 한 번만 호출됨.
  // ─────────────────────────────────────────────
  const applyIllustrationToSlots = useCallback(
    async (
      ...
      const illustrationSlots = tpl.layout.slots.filter(
      ...
      const nextPaths = { ...currentPaths };
      const nextUrls = { ...currentUrls };
      illustrationSlots.forEach((slot) => {
        const src = currentPaths[slot.id];
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const hasIllustrationSlot = template.layout.slots.some(
        (s) => s.auto_illustration,
      );
      if (hasIllustrationSlot) {
        const illustrationResult = await applyIllustrationToSlots(
          template,
          paths,
          urls,
        );
        paths = illustrationResult.paths;
        urls = illustrationResult.urls;
      }
      setImagePaths(paths);
      setImageUrls(urls);
  ...
  return (
    <main className="px-5 py-5 space-y-5">
      ...
      {(() => {
        ...
        const hasIllustration = template.layout.slots.some(
          (s) => s.auto_illustration,
        );
