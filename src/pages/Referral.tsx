...
import { ArrowLeft, Copy, Check, Gift, Users, Loader2 } from "lucide-react";
...
const Referral = () => {
  ...
  const handleShare = async () => {
    const code = await ensureCode();
    if (!code) return;
    const shareText = `Dewy 웨딩플래너에서 같이 결혼 준비해요! 가입 시 제 초대코드 ${code}를 입력하면 500P를 받을 수 있어요. https://www.dewy-wedding.com`;
    if (navigator.share) {
      try {
        await navigator.share({ title: "Dewy 친구 초대", text: shareText });
      } catch {
        // 사용자 취소
      }
    } else {
    ...
    <div className="min-h-screen bg-background max-w-[430px] mx-auto relative pb-20">
      <header className="sticky top-0 z-40 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="flex items-center gap-3 px-4 h-14">
          ...
          <h1 className="text-lg font-bold text-foreground">친구 초대</h1>
        </div>
      </header>
