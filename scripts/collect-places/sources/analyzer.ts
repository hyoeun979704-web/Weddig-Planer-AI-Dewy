...
- avg_price_estimate: 후기에서 가격 단서를 적극 추정하라. 직접 가격이 없어도 "비싸다/저렴하다/100만원대" 같은 표현으로 합리적 범위를 추정. 정말 단서가 0이면 null.
  ★ unit/금액은 카테고리마다 화면에 표시되는 기준(라벨)에 맞춰라. 무조건 1인 환산하지 말 것:
    웨딩홀 → 1인당 식대 (unit=per_person, 보통 6-15만원)
    청첩장 → 1인당 식대 (unit=per_person, 보통 5-15만원)
    메이크업 → 신부 1인 기준 (unit=per_person, 보통 30-80만원)
    스튜디오 → 촬영 패키지 총액 (unit=per_package, ÷2 하지 말 것, 보통 80-300만원)
    드레스 → 1벌 대여 기준 (unit=per_set, 보통 50-300만원)
    한복 → 1세트 대여/맞춤 (unit=per_set, 보통 15-40만원)
    예복 → 신랑 1세트 (unit=per_set, 보통 30-150만원)
    허니문 → 커플 패키지 총액 (unit=per_package, ÷2 하지 말 것, 보통 300-1500만원)
    혼수 → 대표 품목/풀세트 가격 (unit=per_set, ÷2 하지 말 것, 보통 500-2000만원)
    예물 → 커플 세트 또는 대표 단가 (unit=per_set)
