// AI 메이크업 추천 — gpt-image 한 번으로 끝.
//
// 흐름:
//   1. 인증 → 입력 검증 (source_image_path, scene_code)
//   2. 프롬프트 서버 조립 — 클라이언트 prompt 는 받지 않는다(신뢰 경계).
//      개인화(퍼스널컬러·립톤 등)는 style_preference 구조화 슬롯(살균)으로만 반영.
//   3. spend_hearts(5, "makeup_recommend") → makeup_fittings row(mode=recommend, pending)
//   4. gpt-image(셀카 1장 — 모델이 얼굴 분석+메이크업 디자인+적용을 한 번에) →
//      makeup-results 업로드 → done (백그라운드, 202 즉시 반환 — dewy-fitting 패턴 통일)
//   5. 실패 시 환불
//
// 카탈로그 시뮬(dewy-makeup)과 다른 점: 참조 메이크업 이미지 없음(셀카 1장만).
...
serve(async (req) => {
  ...
  try {
    ...
    const prompt =
      buildRecommendMakeupPrompt(body.scene_code as MakeupSceneCode, { retouch }) +
      buildMakeupStyleAddendum(body.style_preference);
    ...
    const { markFailed, refund } = makeJobFailureHandlers({
      ...
      userId,
      amount: HEART_COST,
      earnReason: "refund_failed_makeup_recommend",
    });
