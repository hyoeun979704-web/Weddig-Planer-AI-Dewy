...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const paidAmount = approveData?.amount?.total ?? 0;
    ...
    // 포인트 차감 (실패해도 카카오 결제는 이미 완료된 상태이므로 진행)
    let pointsSpent = 0;
    if (pointsToSpend > 0) {
    ...
    // 하트 적립 — package 매핑 + 첫 충전 reason 구분
    const isStarter = packageId === "starter";
    const { data: earnData, error: earnError } = await supabase.rpc("earn_hearts", {
      p_user_id: userId,
      p_amount: pkg.hearts,
    ...
    if (earnError) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: "결제는 승인됐으나 하트 적립에 실패했습니다. 고객센터 문의해주세요.",
          payment: approveData,
        }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
