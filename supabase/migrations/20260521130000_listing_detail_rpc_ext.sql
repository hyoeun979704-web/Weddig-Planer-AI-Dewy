-- 카테고리별 상세 RPC 확장 — honeymoon/jewelry/appliance 분기 추가.
-- 기존 7종(wedding_hall/studio/dress_shop/makeup_shop/hanbok/tailor_shop/
-- invitation_venue) 유지 + 3종 추가. CREATE OR REPLACE 로 전체 재정의.
...
BEGIN
  ...
  ELSIF v_place.category = 'tailor_shop' THEN
    ...
    VALUES (v_place.place_id, _jsonb_to_text_arr(d->'suit_styles'), (d->>'custom_available')::boolean, (d->>'fitting_count')::int)
    ON CONFLICT (place_id) DO UPDATE SET suit_styles=EXCLUDED.suit_styles, custom_available=EXCLUDED.custom_available, fitting_count=EXCLUDED.fitting_count;
