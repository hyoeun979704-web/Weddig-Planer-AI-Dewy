...
## 4. 데이터 모델 결정 (열린 질문 — 사용자 확인 필요)

1. **추천일정 적용 범위**: 이미 일정을 시드한 기존 사용자에게 (a) 추천 표시에만 반영 / (b) 재시드 제안
   / (c) 신규 사용자만. → **회귀 위험**이라 기본은 (a)+(c) 권장. 확인 필요.
2. **예산 카테고리**: 페르소나 부적합 카테고리를 **숨김** vs **접힘(디엠퍼시스)**. 숨김은 강하지만
   "그래도 쓰고싶다" 케이스 차단 → **접힘 + 되돌리기** 권장.
3. **value_tags 선행 시딩**: ✅ 확인 완료 — 시드 거의 0(친환경1/비건0/반려2/영문0). **P3 코드 보류**,
   데이터 태깅 패스가 선행. AI 프롬프트 주입은 데이터 무관하게 이미 동작.
4. **저장 위치**: 페르소나 프로파일은 **코드(personaPlanProfile.ts)** 단일 소스 권장(운영부담 0, 테스트
   가능). DB 테이블은 A/B·비개발자 편집 필요해질 때 후속.
