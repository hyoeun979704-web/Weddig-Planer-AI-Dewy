...
### Phase 1.5 — 자동발행 + 발행 전 분석·개선 레이어 (AIO 게이트, 같은 PR #499)
- **개선 레이어**(`blog-draft-generator` Stage 3): 작성+자가분석 후 **AIO 미달(점수<85 또는 체크 실패)이면 1회 개선
  재작성+재분석**(약점·미달항목 집어 교정, 리서치 밖 수치 삭제). 자가교정 후 게이트 판정.
- **AIO 자동발행 게이트** `passesPublishGate`: **점수≥75 · no_fabrication · TL;DR · FAQ · 출처≥1** 전부 통과해야 자동 공개.
  미달이면 `status='review'` 보류 + `last_error` 에 사유 기록(사람이 검수). = "빠르되 위험한 글은 사람이".
- autoPublish 시 서버에서 `marked`(esm.sh)로 content_html 렌더 + `slugify`(한글 보존)+중복 회피 후 `status=published`.
- 어드민 "AI 생성" 다이얼로그에 **자동 발행 토글(기본 ON)**. 결과 토스트로 공개/보류 사유 표시.
- ⚠️ 게이트의 점수·체크는 **모델 자가보고**(과대평가 가능) → 객관 신호 `출처≥1` 병행으로 보강. 추가 경성화(적대적 심판 1콜)는 후속 여지.
- esbuild·build·lint·integrity(0) 통과. blog-draft-generator 재배포는 merge 시 자동.
