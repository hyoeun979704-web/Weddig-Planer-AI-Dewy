...
## TL;DR
- **P0 0 · P1 1건(수정 완료) · P2 이월.** 신규 코드는 전부 **읽기/prefill/순수계산** — 신규 DB 쓰기 0,
  신규 RLS 표면 0(보안 회귀 없음). 결제/송금/거래 코드 신설 0(법적 안전 1차 원칙 준수).
- **[P1 수정] 적대적 리뷰 발견**: `HomeExpoDealsRow` 의 로컬 카테고리 매핑이 5개만 커버 →
  예복(tailor_shop)·한복·청첩장 등 페르소나(약 8모드)는 매칭 0이 되어 개인화가 조용히 무력화.
  **수정**: `usePartnerDeals.PLACE_TO_DEAL_CAT` 를 export 해 단일 소스로 쓰고, 매핑 밖 카테고리는
  deals 와 동일하게 `general` 폴백(HomeExpoDealsRow·usePartnerDeals).
...
### 3-perf. 성능 (P2 이월)
- `usePartnerDeals` 는 React Query 래핑 없이 useState/useEffect 로 3쿼리(deals·events·coupons)를
  마운트마다 실행 — **기존 훅 설계**(`/deals`·`/events` 동일). 홈에 얹으며 홈 진입마다 재조회 가능성.
  현재 파트너 데이터 ~0이라 즉시 빈 응답이나, 데이터 증가 시 React Query 캐시 래핑 권장(전 surface 공통).
  이번 세션 회귀 아님(공유 훅) → 별도 리팩터로 이월.
...
---
*P0 0·P1 1(수정)·P2 이월. 신규 쓰기·RLS·결제 표면 0(안전 1차). 빌드·lint·integrity·테스트(1283) 녹색.
커밋: 57556c7·bc65298·8225fef·9e828d5 + P1 수정. 적대적 리뷰 서브에이전트로 P1 발굴·교정.*
