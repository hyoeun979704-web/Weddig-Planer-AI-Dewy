...
> ✅ **최종 채택 워크플로우는 `docs/makeup-samples-final-prompts.md` 참고.**
> 헤어·포즈 드리프트/눈썹 누락을 막기 위해, `avatar-base.png`로 **컨셉별 쌩얼 아바타 6장**을
> 먼저 만들고(헤어·포즈를 아바타에 박아둠) → 각 아바타에 **"헤어·포즈 잠금 + 메이크업만 추가"**
> 항목별 체크리스트로 적용한다. 아래 텍스트 템플릿보다 그쪽이 우선.
