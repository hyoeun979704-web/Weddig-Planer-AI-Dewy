...
등록 신청을 마치면 **"등록 검토 중이에요"** 화면이 나옵니다. 이건 정상이에요.
**운영자가 검토 후 승인**하면, 그때부터 업체 정보 입력·관리 기능이 열립니다.
...
| **사진/메뉴 관리** | 갤러리 사진(또는 메뉴) 업로드 | 검토 후 노출 |
| **쿠폰 관리** | 할인 쿠폰 발행 | ⚡ **즉시 노출** |
...
- **쿠폰**: 발행하면 검토 없이 **즉시** 상세페이지에 노출 → 가장 빠른 고객 유인책.
- **사진/메뉴**: 대표 이미지 외 갤러리를 채울수록 신뢰도가 올라갑니다 (검토 후 노출).
- **이벤트·상품**: 시즌 프로모션·패키지를 등록 (검토 후 노출).
...
| **"등록 검토 중" 화면에서 멈춰 있어요** | 정상입니다. 운영자 승인 후 자동으로 대시보드가 열립니다. |
| **제휴 신청 버튼이 없어요** | ③ 업체 정보 6개 필수항목을 모두 채워야 신청 버튼이 활성화됩니다(`업체 정보 채우러 가기`). |
| **저장했는데 바뀐 정보가 상세페이지에 없어요** | 정보 수정은 **검토 후 반영**됩니다(쿠폰만 즉시 노출). 검토를 기다려 주세요. |
