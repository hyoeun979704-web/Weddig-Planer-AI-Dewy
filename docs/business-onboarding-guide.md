...
| 최소 가격(원) | 검색·견적 **비교/필터**에 사용됩니다 (상세페이지 첫 화면에 크게 보이는 가격은 아래 💰 참고) |
...
| 사장님이 입력 | 고객 화면에서 |
|--------------|--------------|
| **상품/패키지 가격** (상품 관리) | 첫 화면 상단 + 하단 고정바에 **`최저 OOO만원~`** 으로 크게 노출 ✨ |
...
> 💰 **첫 화면의 큰 가격(`최저 OOO만원~`)은 [상품 관리]에 등록한 패키지 가격에서 나옵니다.**
> 위 표의 "최소 가격(원)" 칸만 채우면 검색·필터에는 쓰이지만 **첫 화면 대표 가격으로는 안 보입니다.**
> 상품/패키지를 가격과 함께 1개 이상 등록하세요. 하나도 없으면 `가격은 문의로 안내해드려요` 로 표시됩니다.
>
...
| **첫 화면에 가격이 `문의로 안내`로만 떠요** | 첫 화면 대표 가격은 **[상품 관리]의 패키지 가격**에서 나옵니다 → 가격을 넣은 상품/패키지를 1개 이상 등록하세요(③-1 💰). "최소 가격" 칸은 검색·필터용입니다. |
