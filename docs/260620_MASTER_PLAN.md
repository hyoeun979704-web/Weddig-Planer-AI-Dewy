...
---
## 6. 미해결 / 사용자 입력 대기
- **U1 (결제 전략)**: 디지털 재화를 (A) **네이티브 IAP 30%**(현 확정) 유지 vs (B) **한국 대체결제로
  카카오페이 인앱**(수수료↓·구현/심사 복잡) — 현재 A로 진행. B 검토 원하면 별도 지시.
- **U2 (D14)**: AI 결과 UX 별도 설계 **문서 위치** 공유 → 본 계획과 정합 확인.
- **U3**: 무료체험 **자동결제 고지 문구 ↔ 실제 동작** 확인(UpgradeModal "자동결제 없음" vs 1개월 자동전환).
