...
- **U2 (AI 결과 UX) = 검증 완료 + 범위 확정**(현 코드 6패턴 대조, 성숙도 ~47/100):
  - ✅ 이미 충족: 재생성, 히스토리/갤러리.
  - **구현 확정(3)**: ① **공유 버튼 통일**(헤어·포토픽스·컨설팅에 추가) ② **전후 비교**(원본↔결과)
    ③ **확대/풀스크린**(결과 탭 → 줌 모달).
  - **보류**: 변형(4-up)·즐겨찾기·HD 업스케일/업셀(아직 단계 아님).
  - **구현 방침(DRY)**: 결과 페이지 6종이 동일 패턴 → 공유 `useShareResult` 훅 + `<AiResultImage>`
    (줌+전후비교) 컴포넌트로 **단일화**해 6 페이지가 재사용(드리프트·중복 차단). 모두 웹 UI라
    **이 환경에서 끝까지 검증 가능**. 쉬운순=확대 → 전후비교 → 공유통일.
