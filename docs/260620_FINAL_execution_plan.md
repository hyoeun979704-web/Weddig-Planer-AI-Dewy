...
- **앱 스토어 = 무료 기간엔 IAP 없이도 제출 가능** ✅(갱신): **네이티브 빌드에서 디지털 결제 UI를 숨김**
  (reader 모델 — 인앱 디지털 판매 0 → Google 빌링 위반·Apple 3.1.1 트리거 안 됨). 하트·구독은 웹 판매,
  앱은 잔액 사용만(웹 안내/링크 없음 = anti-steering 안전).
  - **남은 게이트**(외부): Apple 개발자 승인(심사 중)·Data Safety 콘솔 입력(표 제공)·iOS SiwA.
  - **Android**: 가드 반영 빌드로 사실상 제출 가능. **iOS**: Apple 승인 후.
- **IAP(R1)**: 이제 출시 차단이 아니라 **"앱 내 정식 결제를 켤 때"의 후속 작업**(KakaoPay 정기결제·샌드박스 후).
- **결론**: 무료 기간엔 **웹·앱 모두 출시 경로 열림**. 유료 인앱 결제를 켜는 시점에 IAP 전환.
