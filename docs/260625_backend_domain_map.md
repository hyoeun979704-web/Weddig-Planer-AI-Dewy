...
### shared (10) — 인프라 · 결제검증 · 웹훅 (여러 도메인/외부)
...
**요약**: consumer 34 · partners 2 · console 11 · shared 10 = **57**. → **edge function 무게중심은
consumer(AI·청첩장·결제·동기화)와 console(마케팅·수집)**이고, partners 는 거의 PostgREST+RLS 로 동작
(edge 2개)함이 드러난다.

> **정리 이력(2026-06-25)**: 데드/스텁 함수 3개 제거(60→57) — `dewy-studio`(연결 안 된 프로토타입),
> `ask-gemini`(deprecated 410 스텁), `invitation-extract-layout`(index.ts 없는 placeholder → 설계
> 스펙은 `docs/invitation-extract-layout-spec.md` 로 보존). 호출처 0 확인 후 제거.
...
4. **미완/폐기 함수 정리 완료(2026-06-25)** — `invitation-extract-layout`·`dewy-studio`·`ask-gemini`
   3개 제거(호출처 0 확인). `invitation-extract-layout` 설계 스펙은 `docs/invitation-extract-layout-spec.md`
   로 보존. (60→57)
...
> **후속 진행(2026-06-25~)**: §4 deferred 를 순차 PR 로 진행 중 — ① 데드/스텁 함수 정리 ✅(완료, 위 §4-4)
> ② types.ts 드리프트 재생성 ③ 데이터접근 레이어 추상화(도메인별 점진). 물리 함수 폴더 이동은 Supabase
> 배포 제약으로 영구 보류(§0).
