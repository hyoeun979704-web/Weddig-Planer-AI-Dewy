...
6. **C-6 Console 앱 패키징**(Phase 4): 모바일 우선 → **네이티브 or PWA**(아래 미결정).

---

## 6. 미결정 — Console 출시 형태 (솔로 + 폰)

"혼자 + 폰"이면 모바일 우선은 확정. 네이티브 vs PWA는 결정 필요:
- **PWA(권장 후보)**: 설치형 웹, 푸시(웹푸시) 가능, 스토어·심사 0, 즉시 배포. 솔로 내부도구에 적합.
- **네이티브(Capacitor)**: 소비자/partners와 같은 툴체인 재사용, 네이티브 푸시(신고·문의 즉시 알림) 강력.
  단 스토어 공개 대신 내부배포(TestFlight/내부 트랙)로 충분. 심사 부담 회피 가능.
- 둘 다 단일 Supabase 백엔드. 결정만 하면 §5 C-6에 확정 반영.
