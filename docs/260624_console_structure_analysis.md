...
**반드시 지킬 것(차원 연계)**:
- **§10 법적 — 광고 표기(콘텐츠 종류별 조건부, 2026-06-24 정정)**: 표기는 **단독 업체 광고
  (`vendor_ad` = 유료·제휴 = 경제적 이해관계 있음)** 에만 강제(캡션 고지 + 해시태그 #광고#제휴 + 카드 라벨).
  **정보성·큐레이션(`editorial`)은 광고가 아니므로 표기 안 함**(내가 정보를 주는 콘텐츠). 구현:
  `src/lib/marketing/adDisclosure.ts`의 `applyDisclosure(kind, …)` — vendor_ad 만 강제, editorial 원본 유지.
  **안전장치**: 표기 의무 기준은 '경제적 이해관계'다. 정보성처럼 보여도 대가·제휴가 끼면 vendor_ad 로
  분류해야 한다(공정위 '뒷광고' 회피 금지).
...
**콘텐츠 노출 대상(surface) — 결정됨: (a) 외부 채널만(1차, 2026-06-24)**:
- **(a) 외부 채널**(인스타·블로그 등) ✅채택: 기존 instagram 파이프라인 흐름. 광고표기는 캡션/카드 라벨.
  스토어 정책 영향 없음.
- **(b) 소비자 앱 내 스폰서 추천**: 1차 제외. 추후 별도 기획(소비자 앱 변경 + §10 인앱 광고표기 + §8
  스토어 광고정책 + ATT 동반).

**브랜드 자료 출처 — 결정됨: Google Drive 폴더 읽기(2026-06-24)**:
- 업체가 지정한 Drive 폴더를 읽어 학습 → `drive.readonly` 권한 필요.
- ⚠️ **블로커**: `drive.readonly`는 Google **제한범위(restricted scope) 보안심사(CASA)** 대상 — 승인 전엔
  실사용자 동작 불가(심사 기간·비용 발생). 이 부분은 **외부 심사 완료가 출시 전제**.
...
4. **C-4 모바일 우선 재편** (진행 중): ✅ AdminLayout 헤더에 **모바일 그룹 칩 네비**(가로 스크롤, 드로어
   안 열고 6그룹 전환, 터치 타깃 ≥44pt, lg+ 숨김) 추가 — 솔로+폰 전제. 빌드·린트 녹색. ⏳ 추가 폰
   최적화(테이블→카드 변환 등)는 점진.
5. **C-5 마케팅 모듈** (진행 중):
   - ✅ 광고표기 강제 유틸 + 테스트(`src/lib/marketing/adDisclosure.ts`).
   - ✅ **키워드 카드뉴스 품질 업그레이드**(2026-06-24): 골든 샘플(표지 제목+부제목 / 본문 항목명+설명
     +**TIP** / 마무리 질문+공유CTA) 기준으로 `instagram-draft-generator` 프롬프트 재작성 + `instagram-
     card-renderer` 본문 카드에 TIP(footer) 블록 추가. esbuild 번들 검증. ⚠️ LLM 출력·Satori 렌더 실제
     품질은 실환경(배포 후) 확인 필요 — 이 환경 e2e 불가.
   - ⏳ 일괄 생성·렌더 UI·프리뷰(인스타 파이프라인 미완 부분), (선택)6채널.
   - ⛔ 업체 브랜드 학습/페르소나 광고: Drive readonly **Google 보안심사(CASA) 통과 전 출시 불가**(블로커).
