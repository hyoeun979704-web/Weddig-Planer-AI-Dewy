...
## 16. P1 진행 상태 (260625 후속)

| P1 | 상태 | 위치 |
|---|---|---|
| gmail-send 헤더/MIME 살균 | ✅ 머지(#424) | `gmail-send/index.ts`(safeName·safeMime 화이트리스트) |
| Recommend null 가드 + 허위 toast | ✅ 머지(#424) | `dewy-{dress,makeup}-recommend` 에러응답 제네릭화 + FE fittingId 가드 |
| Favorites 잘못된 매핑 / aria-label | ✅ 머지(#424) | `Favorites.tsx`·`Influencers.tsx` |
| `vendor_deliveries` 수신자 컬럼잠금 트리거 | ✅ 이번 PR | `260625130000_vendor_deliveries_recipient_guard.sql` — 수신자는 status/received_at 만 변경(나머지 동결). business_profiles(#429) 동일 관용구(service_role=auth.uid() IS NULL) |
| 차단목록(Block list) 화면 + 해제 연결 | ✅ 이번 PR | `features/consumer/pages/BlockedUsers.tsx` + `/settings/blocked` 라우트 + `MenuSection`(계정) 진입로. `useUnblockUser` dead code 해소 → App Store 1.2 갭 마감 |
| types.ts admin_review_{product,event} drift | ✅ 이번 PR | `types.ts:6195-6214` 2-arg→3-arg(`p_note?`) 표적 수정(전체 재생성은 단일행 237KB·diff 과대라 deferred 유지) |
| safeLocalStorage 통일 | ✅ 이번 PR(잔여 1건) | `usePageTutorial.ts` 미가드 raw 2건 → `safeLocalStorage`. 나머지 호출처는 이미 try/catch 가드 확인(점검 완료) |
| 미성년자 결제 고지 | ✅ 머지(#423 Checkout) | `Checkout.tsx` 법적 고지(환불·미성년자·약관) |
| earn_hearts 멱등 | ⬜ deferred | 금융 경로 — 환불/적립 ref_id 멱등키 도입은 별도(리스크·가격정책 연계) |

> 마이그레이션(`260625130000`)은 prod 직접 미적용 — 머지 시 배포 파이프라인이 적용.
